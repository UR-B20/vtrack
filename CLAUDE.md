...
|---|---|---|---|
| `POST /recognise` | `X-Device-Token` | multipart `image` (JPEG ≤ 400 KB), `device_id`, `captured_at` (ISO), optional `roi`, optional `seen_plate` (§11, Next vehicle) | `{event_id, plate_norm, plate_display, plate_kind, confidence, checksum_ok, repaired_from, decision, reason, flags, vehicle:{owner_name, org_unit, pass_type, valid_until} \| null, latency_ms, deduped}` |
...
- Overlay the detection box + plate + confidence from the last response for 3 s; show the last read chip and engine status pill; Wake Lock; a hold-to-unlock screen lock so the page can't be closed by accident. **Amended 23 Sep 2026 — see §11 (Capture at A, Pairing):** presence as a fraction of cells, queued-car sessions, no buffered retry.
### 6.3 `/admin`
...
| Capture at A (23 Sep 2026) | `lib/presence.ts`: 64×36 grey ROI thumbnails at 4 Hz, each divided by its own mean; present = a fraction of cells changed (so a motorcycle counts). Nothing is sent until the lane has been seen empty; the empty-lane picture is kept across reloads. A different vehicle settling in the lane — even nose to tail, never empty between — starts a new session. Lighting guard: present 60 s with no plate re-baselines. One frame in flight; a failed send does not count towards the 12. **No buffered retry** (brief §2): the next frame, 2 s later, is the retry, and fresher. A new app version applies only when the lane is empty |
| Pairing (23 Sep 2026) | `uv run vtrack-pair-qr cam-a` on the laptop shows a QR code of `{"vtrack":1,"device","token"}` — JSON, never a link — which `/capture` scans (BarcodeDetector, Chrome on Android); typing the token is the fallback. Capture only in M2; the display is paired in M3 |
| Detector threshold (23 Sep 2026) | `ALPR_DETECTOR_CONF` (default 0.4, fast-alpr's) is config: the small detector's score moves with framing, so the M2 gate benchmark measures read rates against it |
| Deferred to M3 (23 Sep 2026) | §5.3's merge of a confusable variant into the open event: with one frame in flight and sending stopped at a confident answer, a later CHECK cannot overwrite a green. §6.1's "vehicle gone" hold for a DENY: capture stops sending after a confident answer, so `last_read_at` stops moving and a DENY clears 15 s after the last read even if the car is still there — M3 adds a no-image presence ping |
| Typefaces (24 Sep 2026) | Owner's call: the system fonts read as generic. Three faces are bundled with the app (npm `@fontsource`, SIL OFL 1.1), served from its own origin and precached by the service worker, so they work offline and no font host is ever contacted: **IBM Plex Mono** for plates, times, IDs and the console's tracked-caps labels (its dotted zero keeps 0 and O apart on a plate); **Barlow** for UI text (drawn from highway signage); **Barlow Condensed** for the stage verbs and big titles. §3's system fonts stay as the fallback stack. Adds about 200 KB to the one-time download. Android does not ship Courier New, so the tablets had been showing their own default monospace; now every device renders the same faces |
| Next vehicle (24 Sep 2026) | Found at STOP 2: after one car was read, the next was not. A new vehicle was recognised only by how different the lane looks, and a lookalike car pulled in nose to tail, or a printed plate swapped in the same hand, changes only about 1–4 % of the lane thumbnail, under the 6 % that marks a new vehicle. Now a vehicle already answered for is re-read with one frame whenever the lane settles again visibly changed (`lib/presence.ts`, decision 4a): a different plate is a new vehicle, and the same plate or none is the same one. The re-read counts towards the vehicle's 12. Every frame carries `seen_plate`, the plate answered for since the lane was last empty; a read of that plate with no open event writes nothing (`dedupe.py` `Skip`), so a car that stands longer than `DEDUPE_S` is never put back on the display |
| Approved screen (24 Sep 2026) | Owner's call: the ALLOW stage shows the owner's name, unit, pass type and validity under PROCEED, as the canvas (gate-allowed.png) has them. This reverses M0, which showed only "On the approved list" because owner names are personal data (brief §3.10) and the screen can be read from outside the post; mount the screen at B facing the guard, not the lane. With the list not yet synced to the display it still says "On the approved list" |
